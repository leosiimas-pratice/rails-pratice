class Sale < ApplicationRecord
  ...
  #Ideias futuras: Colocar o total da venda!!!

  after_save do
    calc = 0
    #Soma o preço dos produtos vezes a quantidade deles
    self.product_quantities.each {|p| calc += p.product.price * p.quatity}
    ...
    if self.comission.present?
      self.comission.update(value: (calc * 0.1), status: :peding)
    elsif
